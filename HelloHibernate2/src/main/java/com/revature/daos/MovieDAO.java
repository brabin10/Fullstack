...
public class MovieDAO {
	...
	public List<Movie> getMoviesDirectorId(int id) {
		...
		//set the ? to the id sent in the method call. This is how a PARAMETERIZE HQL
		q.setParameter(0,id);
	...
	//Using the Session methos to update (we can do update() or merge(), I like merger because it's  less error prone)
	public void updateWithSessionMethod(Movie movie) {
		
		Session ses = HibernateUtil.getSession();
		
		//all updates and deletes must be wrapped within a TRANSACTION
		//I'm not entirely sure why, but presumably it's to prevent inaccurate date if something fails
		Transaction tran = ses.beginTransaction();
		
		//updates whatever Movie got sent in 
		ses.merge(movie); //how does it know which movie to updte? It checks the PK
	...
	public void updateWithHQL(Movie movie) {
		...
		//Asssign our query syntax to a query object
