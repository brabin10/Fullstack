...
public class Pirate extends Person {
	...
	// Pirate has all the variables of the Person class, as well as this variable
	// unique to pirates
	public int peglegs;

	// Pirate has the talk() method of the Person class, as well as this method
	// unique to pirates
	public String swordFight() {
		return "Pirate chopped ya head off";
	}

	// Method Overriding below---------- Note the @Override annotation, which
	// indicates an overridden method

	// Without Overriding, the Pirate Class would have the exact same talk() method
	// as the Person Class...
	// But here, we declare the same method signature, with a different
	// implementation (aka method body)
	// Why do we do this? So that the Pirate Class can have it's own specific
	// functionality for the talk() method
	@Override
	...
	// Constructors are a great example of METHOD OVERLOADING - methods with the
	// same name, but different arguments

	public Pirate() {
		super(); // This call to super() will call the no-args constructor of the parent class
					// (Person)
